...
class _EditRecipeScreenState extends State<EditRecipeScreen> {
  ...
  bool _mealTypesInitialized = false;
  ...
  void initState() {
    ...
    _caloriesController = TextEditingController(
      text: widget.recipe.caloriesPerPortion?.toString() ?? '',
    );
    _proteinsController = TextEditingController(
      text: widget.recipe.proteinsPerPortion?.toString() ?? '',
    );
    _carbsController = TextEditingController(
      text: widget.recipe.carbsPerPortion?.toString() ?? '',
    );
    _fatsController = TextEditingController(
      text: widget.recipe.fatsPerPortion?.toString() ?? '',
    );
    _prepTimeController = TextEditingController(
      text: widget.recipe.prepTimeMinutes?.toString() ?? '',
    );
    _servingsController = TextEditingController(
      text: widget.recipe.baseServings?.toString() ?? '',
    );
    
    _imageUrl = widget.recipe.imageUrl;
    
    // Normalizar el valor de dificultad para que coincida con las opciones del dropdown
    if (widget.recipe.difficulty != null) {
      final normalizedDifficulty = _normalizeDifficulty(widget.recipe.difficulty!);
      _difficulty = normalizedDifficulty;
    }
  ...
  // Normalizar valores de dificultad de la base de datos
  String? _normalizeDifficulty(String difficulty) {
    final normalizedValue = difficulty.toLowerCase().trim();
    switch (normalizedValue) {
      ...
      case 'dificil':
        return 'Difícil';
      default:
        // Si no reconocemos el valor, devolvemos null para que no cause error
        return null;
    }
  }
  ...
  void _updateRecipe() {
    if (_formKey.currentState!.validate()) {
      ...
      context.read<AdminRecipesCubit>().updateExistingRecipe(
        widget.recipe.id,
        recipe,
        _selectedMealTypeIds,
      );
    }
  }
      ...
      body: BlocListener<AdminRecipesCubit, AdminRecipesState>(
        listener: (context, state) {
          if (state.status == AdminRecipesStatus.success) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(state.successMessage ?? 'Receta actualizada'),
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(state.errorMessage ?? 'Error al actualizar receta'),
                ...
                children: [
                  ...
                  BlocBuilder<MealTypesCubit, MealTypesState>(
                    builder: (context, mealTypesState) {
                      ...
                      // Inicializar los tipos seleccionados una sola vez cuando los datos estén listos
                      if (!_mealTypesInitialized && mealTypes.isNotEmpty && widget.recipe.types.isNotEmpty) {
                        _mealTypesInitialized = true;
                        // Buscar los IDs correspondientes a los tipos de la receta
                        for (var typeName in widget.recipe.types) {
                          final matchingType = mealTypes.where((mt) => mt.name == typeName).firstOrNull;
                          if (matchingType != null && !_selectedMealTypeIds.contains(matchingType.id)) {
                            _selectedMealTypeIds.add(matchingType.id);
                          }
                        }
                      }
